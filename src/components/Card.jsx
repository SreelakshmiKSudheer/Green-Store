...
const Card = (props) => {
  return (
    <div className='w-100 h-100 shadow-emerald-900 border-1 rounded-xl p-5 flex flex-col justify-between items-start bg-white hover:shadow-lg transition-shadow duration-300'>
      <Link to='/product' state={{ product: props }} className='w-full h-full flex flex-col justify-between items-start'>
      <div>
      ...
      <div className='mb-0 w-full flex justify-between items-center mt-4'>
        <button className='mt-4 bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 transition-colors' onClick={() => alert(props.title + " added to your cart")}>
          Add to Cart
        </button>
